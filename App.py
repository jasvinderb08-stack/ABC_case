...
package_weight = st.slider('Package Weight (kg)', 0.0, 50.0, 20.0)
...
if st.button('Predict Delivery Delay'):
    ...
    if prediction == 1:
        st.error(f'Prediction: Delivery Delay Likely (Probability: {prediction_proba[1]:.2f})')
    else:
        st.success(f'Prediction: No Delivery Delay (Probability: {prediction_proba[0]:.2f})')
